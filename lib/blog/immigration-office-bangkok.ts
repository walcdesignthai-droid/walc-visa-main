...
export const IMMIGRATION_OFFICE_BANGKOK: Article = {
	...
	bodySections: [
		...
		},
		{
			...
			items: [
				...
				"予約システムの挙動(時間枠を確保するタイプか、当日のキュー番号を確保するタイプか)は窓口・時期で異なる場合があります。",
			],
		},
		{
			...
			items: [
				...
				"窓口: 直接または代理人による届出も可能",
				"オンラインは受付できる期間(期限前の一定日数)が窓口・郵送と異なる運用が一般的です。対象地域・受付期間・必要書類は公式でご確認ください。",
	...
	faq: [
		...
		},
		{
			...
			answer:
				"来所が前提とは限りません。90日レポート(住所の届出)は、オンライン提出・郵送・窓口/代理人といった方法が用意されています。オンラインは受付できる期間が窓口・郵送と異なる運用が一般的です。対象地域・受付期間・必要書類は公式でご確認ください。",
		},
		{
	...
	sources: [
		...
		{
			claim: "90日レポートのオンライン提出・郵送・窓口/代理人という届出方法",
			source: "タイ入国管理局 90日レポート案内 / TM47 オンラインシステム",
		},
		{
			claim:
	...
	placeholders: [
		{
			...
			note: "受付時間・整理券(ウォークイン)の締切時刻は変動しうる。公開直前に公式 Contact ページで再確認し、変更があれば本文・FAQの時刻を更新。",
		},
		{
			key: "online-appointment-scope",
			note: "Online Appointment の対象手続き・対象範囲・1日の枠数・時間枠確保かキュー番号確保かの挙動。公式予約ポータルで対象を再確認のうえ具体化(現状は一般化して記載)。",
		},
		{
			key: "service-floor-by-procedure",
			note: "B棟内の手続き別受付フロア(90日レポート/居住/延長/再入国 等)。フロア配置は変更されうるため、確定情報が取れた場合のみ具体フロアを追記。",
		},
		{
			key: "90day-online-coverage-area",
			note: "90日レポート オンライン提出の対象地域・受付可能期間(期限前の日数)。公式案内で対象を確認し、必要に応じ本文を更新。",
		},
		{
			key: "access-pink-line-detail",
			note: "MRTピンクライン駅からB棟までの徒歩/シャトル詳細(出口番号・スカイウォーク・シャトル運行間隔)は現地運用で変わりうる。一次/現地最新で確認のうえ詳細化。",
		},
	],
};
