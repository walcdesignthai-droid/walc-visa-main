...
export const NON_B_VISA_EXTENSION: Article = {
	...
	placeholders: [
		...
		},
		{
			...
			note: "延長・ワークパーミット更新の推奨前倒し日数(例: 30〜60日前)は媒体により記載が分かれる。公式一次出典で確定値が取れないため本文では「期限前・余裕をもって」と一般化。",
		},
		{
			key: "non-b-foreign-employee-ratio-capital",
			note: "外国人1名あたりの会社資本金要件・タイ人従業員比率などの延長付帯条件は管轄運用・例外(BOI等)で変動。確定値が一次出典で取れないため本文に数値を記載せず保留。",
		},
	],
};
