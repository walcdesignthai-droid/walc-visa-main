 ...
 *
 * verify(2026-06): O-A は MoPH 2021-10-01 告示で総額300万バーツ(USD10万)が現行標準。
 *    一部公館は旧基準(入院40万/外来4万バーツ)を運用。タイ国内で切替/延長する
 *    Non-O(退職)は保険必須の対象外という整理が一般的。出典は references / sources 参照。
 * ----------------------------------------------------------------------------
 */
...
export const RETIREMENT_HEALTH_INSURANCE: Article = {
	...
	bodySections: [
		...
		},
		{
			...
			items: [
				...
				"COVID-19 を含む補償など、証券に明記すべき項目を求められることがあります。証券の記載要件は時期で変わるため、最新の必要記載を確認してください。",
	...
	placeholders: [
		{
			key: "oa-coverage-current-figure",
			note: "Non-OA の現行補償額(総額300万バーツ/USD10万 と 旧 入院40万・外来4万 のどちらが、どの公館・時期に適用されるか)を、申請先公館・タイ公衆衛生省告示の一次情報で最終確認のうえ確定する。公開前に primaryPending を解消。",
		},
		{
			key: "oa-policy-statement-requirements",
			note: "証券に必要な記載(COVID-19 補償の要否、指定様式の有無)とタイ認定保険会社(TGIA リスト)加入の要否を、申請先公館の現行案内で確認して確定する。",
		},
		{
			key: "non-o-insurance-exemption-confirmation",
			note: "タイ国内 Non-O(退職)切替・延長で保険が不要という運用が、管轄イミグレーションの現行案内で確認できるかを一次情報で再確認する(将来の制度変更に注意)。",
		},
	],
};
