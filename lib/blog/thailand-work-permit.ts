...
export const THAILAND_WORK_PERMIT: Article = {
	...
	answerFirst: [
		...
		"申請・更新はオンライン化が進んでおり、雇用局の e-Work Permit システム(eworkpermit.doe.go.th)が中心です。紙での提出は、システム障害などの場合に限り経過的に認められる扱いとされています(取り扱いは変更されうるため最新を公式で確認)。",
	...
	bodySections: [
		...
		},
		{
			...
			items: [
				...
				"健康診断書は、タイの法令で定められた特定の疾患(ハンセン病・第3期梅毒・結核・象皮病・薬物中毒・慢性アルコール中毒)に該当しないことの証明が求められます。発行からの有効期間は短く設定されているため、申請時期に合わせて取得します(具体の日数は要確認)。",
		...
		},
		{
			...
			items: [
				"雇用局は e-Work Permit システム(公式ポータル:eworkpermit.doe.go.th)を導入し、新規・更新・取消などをオンラインで扱う運用に移行しています。",
				"オンライン申請後、全国の指定サービスセンターで本人確認(生体情報の登録)や許可証カードの受領を行う運用が案内されています。来訪が必要な手順・所要は管轄により異なります。",
				"紙(マニュアル)での提出は、システム上の技術的問題がある場合などに限って経過的に認められる扱いと案内されています。経過措置の期限・条件は変更されうるため、申請前に公式の最新案内を事前に確認してください。",
	...
	faq: [
		...
		},
		{
			...
			answer:
				"一般に、タイの医療機関が発行する健康診断書(一定の疾患に該当しないことの証明)が求められます。発行からの有効期間は短いため、申請時期に合わせて取得します。BOI 奨励企業など、取り扱いが異なる場合があるため公式で確認してください。",
		},
	],
	...
	sources: [
		...
		{
			claim:
				"労働許可の新規・更新・取消はオンラインの e-Work Permit システムで扱う運用に移行(公式ポータル eworkpermit.doe.go.th)。紙提出は技術的問題等の場合に限る経過措置とされ、期限は延長されてきた。",
			source:
				"Department of Employment e-Work Permit portal(eworkpermit.doe.go.th)/ Fragomen・Clark Hill による制度解説",
		},
		{
			claim:
				"健康診断書は、ハンセン病・第3期梅毒・結核・象皮病・薬物中毒・慢性アルコール中毒に該当しないことの証明が求められ、タイの医療機関が発行する。BOI 奨励企業では取り扱いが異なる場合がある。",
			source:
				"Thai Ministry of Labour 要件に関する解説(thailawyers.com 等)/ BOI 案内",
		},
		{
			claim:
	...
	placeholders: [
		...
		{
			key: "medical-certificate-validity-days",
			note: "健康診断書の発行からの有効日数(出典により30日/60日の記述差あり)。雇用局公式の規定で確定してから日数を本文化する。現状は『短く設定』と一般化。",
		},
		{
			key: "e-work-permit-mandatory-and-manual-deadline",
			note: "e-Work Permit のオンライン完全義務化日と紙提出経過措置の最終期限(延長が繰り返されている)。確定日を雇用局の公式告示で確認してから日付を明記する。",
		},
		{
			key: "restricted-occupations-list",
			note: "外国人に禁止/制限される職種リスト(会計・特定の士業・労働集約的職種等)。雇用局の最新告示で確認のうえ、必要なら別 cluster 化を検討。本文では具体列挙を避けている。",
		},
		{
			key: "processing-time",
