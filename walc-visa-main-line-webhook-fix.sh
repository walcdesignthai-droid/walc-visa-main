#!/bin/bash
# ============================================================================
# walc-visa-main: LINE webhook route.ts typecheck fix
# ----------------------------------------------------------------------------
# @line/bot-sdk v10 の型 export 構造変更に対応:
#   - WebhookRequestBody → webhook.CallbackRequest
#   - event 型を webhook.Event に明示
# ============================================================================
...
echo "→ Overwrite app/api/line/webhook/route.ts (webhook.CallbackRequest 使用)"
...
git add -A
git commit -m "fix(line): use webhook.CallbackRequest from @line/bot-sdk v10

- WebhookRequestBody は v10 で削除
- webhook.CallbackRequest / webhook.Event に変更
- replyToken の存在を 'in' で narrow"

git push

echo ""
echo "============================================================================"
echo "✓ Fix applied + pushed (Vercel auto-deploy 走ります)"
echo "============================================================================"
echo ""
echo "次:"
echo "  1. Vercel Deployment 確認: https://vercel.com/walc-design/walc-visa-main"
echo "  2. デプロイ完了後、LINE Developers Console で Webhook URL 設定:"
echo "     https://walc-visa.online/api/line/webhook"
echo "  3. [検証] ボタンで Success 確認"
echo "  4. スマホで LINE OA に質問送信 → AI 応答確認"
